---
title: "Single Cell RNA-Seq : 01-QC Standard Prep"
...
saline1.data <- Read10X_h5("/Volumes/Foote HD/Data/Single-cell/Single-cell experiments/Cell Ranger Matix Data Files/IAV_mouse_cellranger_02142024/control_v1/filtered_feature_bc_matrix.h5", use.names = TRUE, unique.features = TRUE)
...
saline2.data <- Read10X_h5('/Volumes/Foote HD/Data/Single-cell/Single-cell experiments/Cell Ranger Matix Data Files/IAV_mouse_cellranger_02142024/control_v2/filtered_feature_bc_matrix.h5', use.names = TRUE, unique.features = TRUE)
...
IAV.data <- Read10X_h5('/Volumes/Foote HD/Data/Single-cell/Single-cell experiments/Cell Ranger Matix Data Files/IAV_mouse_cellranger_02142024/flu/filtered_feature_bc_matrix.h5', use.names = TRUE, unique.features = TRUE)
...
#Filtering objects
saline1.filter <- subset(saline1, subset = nFeature_RNA >= 200 & nFeature_RNA <= 7500 & nCount_RNA >= 200 & nFeature_RNA <= 60000 & percent.mt < 15)
saline1 <- saline1.filter
saline2.filter <- subset(saline2, subset = nFeature_RNA >= 200 & nFeature_RNA <= 7500 & nCount_RNA >= 200 & nFeature_RNA <= 60000 & percent.mt < 15)
saline2 <- saline2.filter
IAV.filter <- subset(IAV, subset = nFeature_RNA >= 200 & nFeature_RNA <= 7500 & nCount_RNA >= 200 & nFeature_RNA <= 60000 & percent.mt < 15)
IAV <- IAV.filter
# Post-check
saline1
table(saline1$orig.ident)
...
table(saline1@meta.data$Phase) %>%
  kable(table.attr = "style = \"color: black;\"", caption = "Number of Cells in each Cell Cycle Stage",
...
```{r update_ident}
table(Idents(saline1))
Idents(saline1) <- "orig.ident"
table(Idents(saline1))
table(Idents(saline2))
...
## Scale the data **This step is computationally intensive and can be quite slow.**
...
```{r elbow}
ElbowPlot(saline1, ndims = 100)
ElbowPlot(saline2, ndims = 100)
ElbowPlot(IAV, ndims = 100)
```
...
saveRDS(saline1, "saline1.std_pipeline_predoubletremoval_IAVseq.rds")
saveRDS(saline2, "saline2.std_pipeline_predoubletremoval_IAVseq.rds")
saveRDS(IAV, "IAV.std_pipeline_predoubletremoval_IAVseq.rds")
```
